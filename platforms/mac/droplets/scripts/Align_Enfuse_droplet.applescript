...
-- Harry van der wolf 15 Mar 2008
----------------------------------------------------------------
----------------------------------------------------------------
on FUNC_ABOUT()
	display dialog "Enfuse droplet 0.0.2 by Harry van der Wolf" & return & return & Â
		"This (G)UI wrapper uses enfuse and align_image_stack " & Â
...
----------------------------------------------------------------
on FUNC_AIS_PARAM()
	display dialog "align_image_stack parameters" & return & return & Â
		"# default parameters already set by this script:" & return & Â
		"-p file, output .pto => set to pano.pto" & return & Â
		"-a prefix align images => set to fused" & return & return & Â
		"# other parameters (to be set by you):" & return & Â
		"-e assume input images are full frame fish eye (default: rectilinear)" & return & Â
		"-t num    Remove all control points with an error higher than num pixels (default: 3)" & return & Â
		"-f HFOV   approximate horizontal field of view of input images, use if EXIF info not complete" & return & Â
		"-m        Optimize field of view for all images, except for first. Useful for aligning focus stacks with slightly different magnification." & return & Â
		"-c num    number of control points (per grid) to create between adjacent images (default: 8)" & return & Â
		"-l        Assume linear input files" & return & Â
		"-s scale  Scale down image by 2^scale (default: 1 [2x downsampling])" & return & Â
		"-g gsize  Break image into a rectangular grid (gsize x gsize) and attempt to find 
             num control points in each section (default: 5 [5x5 grid] )" & return & return Â
		buttons {"Ok"}
...
on FUNC_ENFUSE(dropped_on, ImageList)
	...
	---- start of requesting align_image_stack parameters
	set align_image_stack_parameters to ""
	set get_going to "NOK"
	repeat while get_going is "NOK"
		display dialog "align_image_stack parameters" & return & return & Â
			"# default parameters set by this script:" & return & Â
			"-p file, output .pto => set to pano.pto" & return & Â
			"-a prefix align images => set to fused" & return & return & Â
			"# Other parameters can be set by you:" & return & Â
			"(Click Show Parameters to show possibilities)" & return & return & Â
			"Enter your additional parameters below. " default answer align_image_stack_parameters Â
			buttons {"Show Parameters", "Quit", "Ok"} default button "Ok"
		set dialogResult to result
		set align_image_stack_parameters to text returned of dialogResult
		set choice to button returned of dialogResult
		if choice = "Show Parameters" then FUNC_AIS_PARAM()
		if choice = "Quit" then return -- exit script immediately
		if choice = "Ok" then set get_going to "OK"
	end repeat
	--- end of requesting align_image_stack parameters
	
	-- delete possible previous fused intermediate images (normally overwrittenÂ
	...
	do shell script "cd " & workingDir & "; " & workingDir & "align_image_stack" & " -a fused " & align_image_stack_parameters & " " & ImageList
	FUNC_quit_BP_PGB()
	...
	do shell script "cd " & workingDir & "; " & workingDir & "enfuse " & enfuse_additional_parameters & " -o " & NewImage & " " & workingDir & "fused*.tif"
	FUNC_quit_BP_PGB()
