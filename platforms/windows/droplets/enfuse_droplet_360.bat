...
set use_exiftool=1
echo enfuse with exif droplet batch file version 0.3.1
...
if not exist exiftool_arguments.txt (
  echo exiftool_arguments.txt not found - EXIF info will not be copied!
  set use_exiftool=0
)  
...
if %use_exiftool%==1 for /F "usebackq delims=" %%i IN (`collect_data_enfuse.bat %*`) DO (
  exiftool.exe -TagsFromFile "%~1" -@ exiftool_arguments.txt -ImageDescription="%%i" "%~dpn1_enfused.tif" 
)  
        rem that's all
goto :ready
...
:loop
  ...
  if !CNT! GEQ %enfuse_number_of_files% ( 
    ...
    echo enfuse.exe %enfuse_additional_parameters% -o "%enfuse_first_file%_enfused.tif" %enfuse_files%>> $$$_enfuse_temp_$$$.bat 
        rem inform user
    ...
    if %use_exiftool%==1 for /F "usebackq delims=" %%i IN (`collect_data_enfuse.bat %enfuse_files%`) DO echo exiftool.exe -TagsFromFile "%exiftool_first_file%" -@ exiftool_arguments.txt -ImageDescription="%%i" "%enfuse_first_file%_enfused.tif">> $$$_enfuse_temp_$$$.bat
        rem init variables
    set enfuse_files=
    set exiftool_files=
    set CNT=0
