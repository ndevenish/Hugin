...
do
 ...
 env \
  CC=$CC CXX=$CXX \
  CFLAGS="-arch $ARCH $ARCHARGs $OTHERARGs -O2 -g -dead_strip" \
...
if [ -f "$REPOSITORYDIR/lib/libwx_macud-$WXVER_FULL.dylib" ] ; then
  install_name_tool \
...
if [ -f "$REPOSITORYDIR/lib/libwx_macud_gl-$WXVER_FULL.dylib" ] ; then
  install_name_tool \
  ...
  do
    install_name_tool \
      -change "$REPOSITORYDIR/arch/$ARCH/lib/libwx_macud-$WXVER_COMP.dylib" \
        "$REPOSITORYDIR/lib/libwx_macud-$WXVER_COMP.dylib" \
        "$REPOSITORYDIR/lib/libwx_macud_gl-$WXVER_FULL.dylib";
  done
