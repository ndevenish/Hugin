...
Output file. If obmitted, stdout is used.
...
Level horizon (works best for horizontal panos)
...
When B<-a>, B<-l>, and B<-s> options are used together, an operation similar
