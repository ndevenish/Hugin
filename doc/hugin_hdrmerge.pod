...
=item c - choose pixels with heighest weight instead of averaging
(overrides options B<-a> I<b> and B<-a> I<d>)

=item d - choose a pixel with the heighest weight instead of averaging
when all pixel weights are within 10% of eachother

=item h - favor a high signal to noise ratio
