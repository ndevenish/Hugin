...
struct ResponseTransform
{
    ...
    ResponseTransform(const PT::SrcPanoImage & src)
    {
        init(src);
    }

    void init(const PT::SrcPanoImage & src)
    {
//        DEBUG_DEBUG(" " << src.getFilename() << ": resp type:" <<   src.getResponseType() << " expo: " << src.getExposure());
        ...
        if (m_src.getResponseType() != PT::SrcPanoImage::RESPONSE_LINEAR) {
            ...
            {
                case PT::SrcPanoImage::RESPONSE_EMOR:
                    {
                    if (lutLen == 1<<10) {
                    ...
                    break;
                case PT::SrcPanoImage::RESPONSE_GAMMA:
                    m_lutR.resize(lutLen);
    ...
    {
        if (m_src.getVigCorrMode() & PT::SrcPanoImage::VIGCORR_RADIAL) {
...
struct InvResponseTransform : public ResponseTransform<VTIn>
{
    ...
    InvResponseTransform(const PT::SrcPanoImage & src)
