...
class ptCPEvent : public wxCommandEvent
{
    DECLARE_DYNAMIC_CLASS(ptCPEvent)
    
    enum CPEventMode { NONE, NEW_POINT_CHANGED, POINT_SELECTED, POINT_CHANGED, REGION_SELECTED };
public:
    ptCPEvent( );
    /// a new point has been created.
    ptCPEvent(wxWindow* win, wxPoint & p);
    /// a point has been selected
    ptCPEvent(wxWindow *win, unsigned int cpNr);
    /// a point has been moved
    ptCPEvent(wxWindow* win, unsigned int cpNr, const wxPoint & p);
    /// region selected
    ptCPEvent(wxWindow* win, wxRect & reg);    
...
typedef void (wxEvtHandler::*ptCPEventFunction)(ptCPEvent&);

BEGIN_DECLARE_EVENT_TYPES()
    DECLARE_EVENT_TYPE(ptEVT_CPEVENT,1)
END_DECLARE_EVENT_TYPES()

#define EVT_CPEVENT(func) \
    DECLARE_EVENT_TABLE_ENTRY( ptEVT_CPEVENT, \
                            -1,                       \
                            -1,                       \
                            (wxObjectEventFunction)   \
                            (ptCPEventFunction) & func, \
                            (wxObject *) NULL ),
