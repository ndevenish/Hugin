...
IMPLEMENT_DYNAMIC_CLASS( ptCPEvent, wxEvent )
DEFINE_EVENT_TYPE( ptEVT_CPEVENT )

ptCPEvent::ptCPEvent( )
{
    SetEventType( ptEVT_CPEVENT );
    SetEventObject( (wxWindow *) NULL );
    mode = NONE;
}

ptCPEvent::ptCPEvent(wxWindow* win, wxPoint & p)
{
    SetEventType( ptEVT_CPEVENT );
    SetEventObject( win );
...
ptCPEvent::ptCPEvent(wxWindow *win, unsigned int cpNr)
{
    SetEventType( ptEVT_CPEVENT );
    SetEventObject( win );
    mode = POINT_SELECTED;
...
ptCPEvent::ptCPEvent(wxWindow* win, unsigned int cpNr, const wxPoint & p)
{
    SetEventType( ptEVT_CPEVENT );
    SetEventObject( win );
    mode = POINT_CHANGED;
...
ptCPEvent::ptCPEvent(wxWindow* win, wxRect & reg)
{ 
    SetEventType( ptEVT_CPEVENT );
    SetEventObject( win );
...
wxEvent * ptCPEvent::Clone() const
{
    return new ptCPEvent(*this);
}
    ...
    if (mouse->LeftUp()) {
        ...
        case KNOWN_POINT_SELECTED:
            if (point != points[selectedPointNr]) {
                ptCPEvent e( this, selectedPointNr, points[selectedPointNr]);
                emit(e);
...
bool CPImageCtrl::emit(ptCPEvent & ev)
{
