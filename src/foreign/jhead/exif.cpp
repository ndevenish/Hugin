...
//--------------------------------------------------------------------------
static void ProcessExifDir(ImageInfo_t &ImageInfo, unsigned char * DirStart,
                           ...
                           int NestingLevel)
{
    ...
    if (NestingLevel > 4){
        DEBUG_ERROR("Maximum directory nesting exceeded (corrupt exif header)");
        return;
    }
...

    {
        ...
        if (DirEnd+4 > (OffsetBase+ExifLength)){
            if (DirEnd+2 == OffsetBase+ExifLength || DirEnd == OffsetBase+ExifLength){
                ...
                // might trigger this.
                DEBUG_ERROR("Illegally sized directory");
                return;
            }
        }
    ...
    for (de=0;de<NumDirEntries;de++){
        ...
        if ((Format-1) >= NUM_FORMATS) {
            ...
            DEBUG_ERROR("Illegal number format " << Format << " for tag" << Tag);
            continue;
        }
        ...
        if (ByteCount > 4){
            ...
            if (OffsetVal+ByteCount > ExifLength){
                ...
                DEBUG_ERROR("Illegal value pointer for tag " << Tag);
                continue;
            }
        ...
        switch(Tag){
            ...
            case TAG_ORIENTATION:
                ...
                if (ImageInfo.Orientation < 1 || ImageInfo.Orientation > 8){
                    DEBUG_ERROR("Undefined rotation value "<< ImageInfo.Orientation);
            ...
            case TAG_INTEROP_OFFSET:
                {
                    ...
                    if (SubdirStart < OffsetBase || SubdirStart > OffsetBase+ExifLength){
                        DEBUG_DEBUG("Illegal exif or interop ofset directory link");
                    }else{
                        ProcessExifDir(ImageInfo, SubdirStart, OffsetBase, ExifLength, NestingLevel+1);
                    }
                    continue;
                }
        }
            ...
            if (Offset){
                ...
                if (SubdirStart > OffsetBase+ExifLength){
                    if (SubdirStart < OffsetBase+ExifLength+20){
                        ...
                        if (ImageInfo.ShowTags) printf("Thumbnail removed with Jhead 1.3 or earlier\n");
                    }else{
                        DEBUG_ERROR("Illegal subdirectory link");
                    }
                }else{
                    if (SubdirStart <= OffsetBase+ExifLength){
                        ProcessExifDir(ImageInfo, SubdirStart, OffsetBase, ExifLength, NestingLevel+1);
...
//--------------------------------------------------------------------------
void process_EXIF (ImageInfo_t &ImageInfo, unsigned char * ExifSection, unsigned int length)
{
    int FirstOffset;
    ...
    */
    ...
    {   // Check the EXIF header component
        ...
        if (memcmp(ExifSection+2, ExifHeader,6)){
            DEBUG_ERROR("Incorrect Exif header");
            return;
        }
    }

    if (memcmp(ExifSection+8,"II",2) == 0){
        if (ImageInfo.ShowTags) printf("Exif section in Intel order\n");
        MotorolaOrder = 0;
    }else{
        if (memcmp(ExifSection+8,"MM",2) == 0){
            if (ImageInfo.ShowTags) printf("Exif section in Motorola order\n");
            MotorolaOrder = 1;
        }else{
            DEBUG_ERROR("Invalid Exif alignment marker.");
            return;
        }
    }
    ...
    if (Get16u(ExifSection+10) != 0x2a){
        DEBUG_ERROR("Invalid Exif start (1)");
        return;
    }
...
//--------------------------------------------------------------------------
int RemoveThumbnail(ImageInfo_t &ImageInfo, unsigned char * ExifSection, unsigned int Length)
{
...
//--------------------------------------------------------------------------
void ShowImageInfo(ImageInfo_t & ImageInfo)
{
    int a;
    printf("File name    : %s\n",ImageInfo.FileName);
