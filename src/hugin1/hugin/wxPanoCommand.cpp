...
void wxImportPTProjectCmd::execute()
{
    PanoCommand::execute();
#ifdef _Hgn1_PANOCOMMAND_H
    Panorama& pano = o_pano;
#endif
    Panorama new_pano;
    wxLoadPTProjectCmd load(new_pano, filename, prefix);
    load.execute();
    if(new_pano.getNrOfImages()>0)
    {
        vector<unsigned int> new_image_nr(new_pano.getNrOfImages());
        unsigned int oldNrOfImage=pano.getNrOfImages();
        HuginBase::OptimizeVector optVec=pano.getOptimizeVector();
        HuginBase::OptimizeVector optVecNew=new_pano.getOptimizeVector();
        //add only new images
        for(unsigned int i=0;i<new_pano.getNrOfImages();i++)
        {
            std::string filename=new_pano.getImage(i).getFilename();
            bool found=false;
            for(unsigned int j=0;j<pano.getNrOfImages();j++)
            {
                if(pano.getImage(j).getFilename()==filename)
                {
            ...
            if(!found)
            {
                ...
                new_image_nr[i]=pano.addImage(new_pano.getImage(i));
                //copy also optimise vector
                optVec.push_back(optVecNew[i]);
            };
        };
        pano.setOptimizeVector(optVec);
        //now update lens numbers
        HuginBase::StandardImageVariableGroups variable_groups(pano);
        HuginBase::ImageVariableGroup & lenses = variable_groups.getLenses();
        HuginBase::StandardImageVariableGroups new_variable_groups(new_pano);
        HuginBase::ImageVariableGroup & new_lenses = new_variable_groups.getLenses();
        for (unsigned int i=0; i<new_pano.getNrOfImages(); i++)
        {
            if(new_image_nr[i]>=oldNrOfImage)
            {
                ...
                for(unsigned int j=0; j<new_pano.getNrOfImages(); j++)
                {
                    if(i==j)
                        continue;
                    if(new_lenses.getPartNumber(j)==oldLens)
                    {
                        if(new_image_nr[j]<oldNrOfImage)
                        {
                ...
                const HuginBase::SrcPanoImage img=new_pano.getImage(i);
                for (unsigned int j=0; j<oldNrOfImage; j++) 
                {
                    SrcPanoImage other = pano.getSrcImage(j);
                    if(other.getExifModel().size()>0 && other.getExifMake().size()>0)
                    {
                        if (other.getSize()==img.getSize() && 
                            other.getExifModel()==img.getExifModel() && other.getExifMake()==img.getExifMake() && 
                            other.getExifFocalLength()==img.getExifFocalLength())
                        {
                            lenses.switchParts(new_image_nr[i],lenses.getPartNumber(j));
                            lenses.updatePartNumbers();
                            break;
                        }
                    }
        ...
        for (unsigned int i=0; i<new_pano.getNrOfImages(); i++)
        {
            for(unsigned int j=i+1;j<new_pano.getNrOfImages();j++)
            {
                const HuginBase::SrcPanoImage &img=new_pano.getImage(i);
#define image_variable( name, type, default_value )\
                if(img.name##isLinkedWith(new_pano.getImage(j)))\
                {\
                    pano.linkImageVariable##name(new_image_nr[i],new_image_nr[j]);\
                };
...
#undef image_variable
        ...
        //now translate cp
        CPVector cps=new_pano.getCtrlPoints();
