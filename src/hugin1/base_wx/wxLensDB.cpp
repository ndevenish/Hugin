...
void LoadLensDBDialog::SetLensName(std::string lensname)
{
    if(!lensname.empty())
    {
        XRCCTRL(*this,"load_lens_name",wxTextCtrl)->SetLabel(wxString(lensname.c_str(), wxConvLocal));
    };
...
void LoadLensDBDialog::OnSearch(wxCommandEvent & e)
{
    ...
    wxString lensname=XRCCTRL(*this,"load_lens_name",wxTextCtrl)->GetValue().Trim();
    if(lensname.Len()>0)
    {
        m_lenslist->Clear();
        m_lensnames.clear();
        bool fuzzy=XRCCTRL(*this,"load_lens_fuzzy",wxCheckBox)->GetValue();
        if(HuginBase::LensDB::LensDB::GetSingleton().FindLenses(std::string(lensname.mb_str(wxConvLocal)),m_lensnames,fuzzy))
        {
            wxArrayString items;
            for(HuginBase::LensDB::LensDBList::const_iterator it=m_lensnames.begin();it!=m_lensnames.end();it++)
            {
                items.push_back(wxString((*it).c_str(),wxConvLocal));
            };
...
bool ApplyLensDBParameters(wxWindow * parent, PT::Panorama *pano, HuginBase::UIntSet images, PT::PanoCommand*& cmd)
{
    ...
    {
        std::string lensname=dlg.GetLensName();
...
void SaveLensDBDialog::OnOk(wxCommandEvent & e)
{
    ...
    if(GetLensName().empty())
    {
        wxMessageBox(_("The lens name contains only an empty string."),_("Warning"),wxOK|wxICON_ERROR,this);
        return;
    };
...
bool SaveLensParameters(wxWindow * parent, const HuginBase::SrcPanoImage& img, bool includeVignetting)
{
    ...
    if(ShowFileDialogWithWarning(dlg,userDBPath))
    {
        ...
        {
            int e=lensDB.BeginSaveLens(std::string(dlg.GetPath().mb_str(HUGIN_CONV_FILENAME)),lensDlg.GetLensMaker(),
                lensDlg.GetLensName(),lensDlg.GetLensMount(),img.getProjection(),img.getExifCropFactor());
            if(e==0)
            {
