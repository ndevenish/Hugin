...
bool PTBatcherGUI::OnInit()
{
    ...
    static const wxCmdLineEntryDesc cmdLineDesc[] =
    {
     ...
     //        wxCMD_LINE_VAL_NUMBER, wxCMD_LINE_PARAM_OPTIONAL },
      ...
      { wxCMD_LINE_PARAM,  NULL, NULL, _T("<project1 <output prefix>> <project2 <output prefix>> <project3"),
	...
	m_frame = new BatchFrame(_T("Batch Processor"),&m_locale,m_xrcPrefix);
	m_frame->Show(true);
	...
	while(parser.GetParamCount()>count)
	{
		...
		if(!projectSpecified)	//next parameter must be new script file
		{
			...
			projList.Add(proj);
			m_frame->SetStatusText(_T("Added project "+param));
