...
void VigCorrDialog::UpdateDisplayData()
{
    ...
    if (iopts.m_vigCorrMode & ImageOptions::VIGCORR_RADIAL) {
        m_corrFlatRB->SetValue(false);
        m_corrPolyRB->SetValue(true);
    } else if (iopts.m_vigCorrMode & ImageOptions::VIGCORR_FLATFIELD) {
    ...
    m_coef0Edit->SetValue(doubleTowxString(const_map_get(m_pano.getImageVariables(m_imgNr),"Va").getValue(),ndigits));
    m_coef1Edit->SetValue(doubleTowxString(const_map_get(m_pano.getImageVariables(m_imgNr),"Vb").getValue(),ndigits));
    m_coef2Edit->SetValue(doubleTowxString(const_map_get(m_pano.getImageVariables(m_imgNr),"Vc").getValue(),ndigits));
    m_coef3Edit->SetValue(doubleTowxString(const_map_get(m_pano.getImageVariables(m_imgNr),"Vd").getValue(),ndigits));
