/* find_N8_lines.cpp
  ...
  The input imge should be black edges on white background.
  Y runs down, X right in normal viewing orientation.

  edgeMap2linePts() converts the edge map image into a line 
  points map: all white except for connected groups of points 
  that belong to unbranched line segments without sharp corners. 
  Returns number of segments.

  linePts2lineList() converts a line map image to a list of 
  lines that meet some selection criteria. The line list is a
  vector of vectors of integer x,y pairs in vigra Point2D 
  sructs.  The list vector is passed by caller, the point
  vectors are created here. Returns the number of lines added.
...
static void plot_lines( vector< vector< Point2D > > lines, int wid, int hgt, char * fnm ){
typedef vigra::BRGBImage::PixelType RGB;
	int n = lines.size();
	if( n < 1 ) return;
	vigra::BRGBImage tmp(wid, hgt);		
	vigra::initImage(srcIterRange(tmp.upperLeft(),
			tmp.upperLeft() + vigra::Diff2D(wid, hgt)),
                	RGB(255,255,255) );
	for( unsigned iln = 0; iln < n; iln++ ){
	...
	cout << "  debug plot "<<fnm<<"  "<<n<<" lines "<< endl;
	exportImage(srcImageRange(tmp), ImageExportInfo(fnm).setPixelType("UINT8"));
}


int 
edgeMap2linePts( BImage & edge, bool save_images ){

	int width  = edge.width(),
		height = edge.height();
	
	BImage line( width, height );

// debug statistics
	unsigned int npnts = 0, nends = 0, nmids = 0,
				 nelb = 0, nnhisto[15];
	for(int i = 0; i < 15; i++ ) nnhisto[i] = 0;

	BImage::traverser eul, lul;

// erase border pixels of edge image
	eul = edge.upperLeft();
	for(int x = 0; x < width; x++ ){
		eul(x,0) = N8_bg;
		eul(x,height-1) = N8_bg;
	}
...
/* 3-point float vector curvature
   positive inward
*/
inline void vcurv ( Point2D & l, Point2D & m, Point2D & r,
				      float & vx, float & vy ){
...
int 
linePts2lineList( BImage & img, 
				  int minsize,
				  double flpix,  // focal length in pixels
				  vector< vector< Point2D > > & lines
			     )
{
	// debug data
	vector< vector< Point2D > > badlen, badori, badcrv;
	int nadd = 0, nrejL = 0;
			...
			if( cd == N8_end ){	// new line...
			  ...
			  // validate the point list
				...
				if( pts.size() < minsize ){
					...
					int ncopy = 0, nskip = 0;
			  /* bend test
				 compute running chord and arc on a fixed span.
			  ...
			  */
					int ip = 0, np = pts.size();
						
			/*		np / 20;
					if( span < 5 ){ span = np / 10; }
					if( span < 5 ){ span = np / 5; }
					if( span < 5 ){ span = np; }
			*/		
					vector<Point2D> tmp;
					...
					for( ip = 1; ip < np - span; ip++ ){
					  ...
					  */
						if( Arc - Chord <= maxacd ){
							...
							else if( q == 0 ) badlen.push_back( tmp );
							else if( q == -1 ) badori.push_back( tmp );
							else if( q == -2 ) badcrv.push_back( tmp );
				...
				else {	// length ok, no curvature test
					lines.push_back( pts );
					++nadd;
				}
			}  // end trace line
...
/* file debug images
*/
	plot_lines( lines, width, height, "dbg_goodlines.jpg");
	plot_lines( badlen, width, height, "dbg_badlen.jpg");
	plot_lines( badori, width, height, "dbg_badori.jpg");
	plot_lines( badcrv, width, height, "dbg_badcrv.jpg");
